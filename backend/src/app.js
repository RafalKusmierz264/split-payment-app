...
// --- GROUP CLOSE ---
app.post("/api/groups/:groupId/close", auth, async (req, res) => {
  try {
...
// --- GROUP REOPEN ---
app.post("/api/groups/:groupId/reopen", auth, async (req, res) => {
  try {
    const { groupId } = req.params;
    const userId = req.user.userId;
    ...
    group.closedAt = null;
    group.closedByUserId = null;
    if (Object.prototype.hasOwnProperty.call(group, "reopenedAt")) {
      group.reopenedAt = new Date();
    }
    if (Object.prototype.hasOwnProperty.call(group, "reopenedByUserId")) {
      group.reopenedByUserId = userId;
    }
    await group.save();
...
// --- EXPENSES ---
// Contract (create):
// body: { title: string (required), amount: number (required), participantIds?: string[] }
// - paidByUserId is always the authenticated user (req.user.userId)
// - if participantIds missing/empty -> defaults to all group.memberIds
// - splits are auto-calculated on backend (equal shares)
app.post("/api/groups/:groupId/expenses", auth, async (req, res) => {
  try {
    ...
    const paidByUserIdRaw = req.body?.paidByUserId ?? req.body?.paidBy;
    const paidByUserId = paidByUserIdRaw ? String(paidByUserIdRaw) : String(userId);

    const memberSet = new Set([...group.memberIds.map(String), String(group.ownerId)]);
    if (paidByUserIdRaw && !memberSet.has(paidByUserId)) {
      return res.status(403).json({ error: "paidByUserId is not a member of this group" });
    }
